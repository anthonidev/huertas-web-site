const getLocations = (
  ...
  const department = departments.find((d) => d.id === idDepartment);
  const province = provinces.find((p) => p.id === idProvince);
  const district = districts.find((d) => d.id === idDistrict);

  return `${district?.name}, ${province?.name}, ${department?.name}`;
};
...
export { departments, provinces, districts, getLocations };
