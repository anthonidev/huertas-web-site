'use client';
import { ImagePreviewIcon, ObrasIcon, WorkingIcon } from '@/components/icons/Icons';
    ...
    <div className="mx-auto max-w-7xl py-10" id="proyecto-galeria">
      <div className="flex flex-col items-end p-4 text-2xl font-extrabold xl:text-3xl">
        <p className="flex items-center space-x-2">
          <WorkingIcon className="h-16 w-16" style={{ color: secondary_color }} />
          ...
          <AliceCarousel
            ...
            items={gallery.map((item) => (
              <div className="relative mt-5 flex justify-end" key={item.id}>
                <button onClick={() => handleOpen(item.id)} className="absolute right-4 top-4">
                  <ImagePreviewIcon className="h-14 w-14 text-white" />
                </button>
                <Image src={item.image} alt={'gallery'} width={500} height={500} />
              </div>
            ))}
          />
        )}
                ...
                <Dialog.Panel className="relative flex w-full max-w-5xl transform justify-center overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8">
                  {currentImage && (
                    <div className="flex items-center">
                      ...
                      <Image src={currentImage.image} alt={'gallery'} width={800} height={800} />
