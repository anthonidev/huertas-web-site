...
export const BannerPrimary = ({
  ...
  const styleLastWord = words.map((word, index) => {
    if (word === lastWord) {
      return (
        <span key={index} style={{ background: primary_color }} className="px-2">
          {word}
        </span>
      );
    }
  ...
  return (
    <header
      ...
      style={{
        backgroundImage: `url(${primary_banner})`,
    ...
    >
      ...
      >
        <Image src={logo} alt={slogan} width={200} height={200} className="" />
      </JackInTheBox>

      <div className="mt-10 flex border-separate flex-col items-center justify-center space-y-5">
        <Image src={logo} alt={slogan} width={200} height={200} className="block md:hidden" />
        <Fade direction="up">
          <h1
            className={clsx(
      ...
      <div className="mx-5 flex w-full flex-col justify-between space-y-10 md:flex-row md:space-y-0">
        <Fade direction="left" triggerOnce={true} className="flex basis-1/2 flex-col">
          <div className="mr-5 flex basis-1/2 flex-col items-center justify-center text-white">
            <span className="text-sm font-bold uppercase md:text-xl">Lotes en</span>
